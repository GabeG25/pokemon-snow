...
cd ~/pokemon-snow && claude
Then at the Claude Code prompt:
1. CEO/CTO operating-mode one-liner
2. cat ~/pokemon-snow-handoffs/[most-recent].md
3. Run the FIRST ACTION canary verification from the handoff
4. Proceed with the START HERE section

---

## Build Artifacts: Where Compiled Output Goes

When `make -j$(nproc)` runs from the repo root:

1. Source files (`src/**/*.c`, `src/**/*.s`) compile to object files in `build/emerald/src/...`
2. Object files link to `pokeemerald.elf` at repo root
3. `pokeemerald.elf` is object-copied to `pokeemerald.gba` at repo root (the final ROM)
4. Linker map writes to `pokeemerald.map` at repo root

**None of the build artifacts are committed.** The ROM is rebuildable from source at any commit, and canary MD5s in handoffs let us verify correct build output.

**Build time:** Clean build ~5 minutes. Incremental build ~30–60 seconds for most changes. Flag-ceiling expansion and full-rebuild-triggering changes ~3 minutes.

---

## The Toolchain: devkitARM

Installed at `/opt/devkitpro/devkitARM/`. Provides `arm-none-eabi-gcc`, `arm-none-eabi-ld`, `arm-none-eabi-objdump`, `arm-none-eabi-nm`, `arm-none-eabi-objcopy`, etc.

Installation was done via devkitpro-pacman during initial setup (the `install-devkitpro-pacman` file in the repo root is the installer artifact, gitignored).

**Do not modify the toolchain.** If a toolchain issue arises, reinstall; don't patch.

---

## What's NOT in These Locations

Several categories of project content exist only in the CEO's head or in external systems:

- **Long-term design intent** beyond what's captured in v17 (art direction, music direction, specific story beats not yet documented, balance philosophy nuances)
- **Playtest feedback and balance iteration notes** (will exist after playtesting begins, post-map-wiring)
- **Art assets** (sprites, tiles, portraits) — none exist yet beyond vanilla pokeemerald
- **Music and sound** — none exist yet beyond vanilla
- **External references** (Pokédex entries from mainline games, pokeemerald documentation, trainerproc docs) — accessible via web search when needed

When Claude Code needs information outside the filesystem, it should ask the CEO rather than guess.

---

## Cross-Reference: Where to Look for What

**"How do I format a trainer entry?"** → `CLAUDE.md` (8 trainerproc rules) or `tools/snow_port/port_trainers.py` (emitter code)

**"What's R5's trainer list?"** → `design-archive/POKEMON_SNOW_RESUME_HANDOFF_v17.md` §20

**"What was the Skier→Hiker probe?"** → `git log --grep="Skier"` or the commits themselves (`592e85b064`, `43ffd4cfb2`) and the corresponding handoff file

**"Why is the saveblock sized what it is?"** → Commit message body of `5ae831757f` (flag-ceiling expansion). Methodology in `PROJECT_OVERVIEW.md` section "Methodology Patterns."

**"What's the current state of the project?"** → Most recent file in `~/pokemon-snow-handoffs/`

**"What are the locked design decisions?"** → `PROJECT_OVERVIEW.md` section "Design Decisions (Locked)"

**"How does port_trainers.py work?"** → Read the docstring at top of the file, then the function definitions. `CLAUDE.md` has invocation examples.

**"What's the git history of X?"** → `git log --oneline -- path/to/X` or `git log -p path/to/X` for full diff history

**"What does the canary look like right now?"** → Most recent handoff file, "Canary" section

---

## Update Protocol

When a new major location is added to the project (e.g., map files get wired, species data files get populated, external asset folder gets established), add a section to this file and commit the update with a `docs:` commit message. Keep the file current with the actual filesystem structure.

When a file is renamed or removed, update references in this document in the same commit.

Last updated: 2026-04-12 (post-R3 batch).# Pokémon Snow — Project Files Map
