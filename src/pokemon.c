...
// For regional-variant entries whose enum tag includes a form prefix
// (HOENN_DEX_ALOLAN_VULPIX, etc.) but maps to the base species' NATIONAL_DEX.
#define HOENN_TO_NATIONAL_FORM(hoennName, baseName) \
    [HOENN_DEX_##hoennName - 1] = NATIONAL_DEX_##baseName
...
// Assigns all Boralyss Dex indexes to a National Dex index.
// Order kept in lockstep with enum HoennDexOrder in include/constants/pokedex.h.
// Use HOENN_TO_NATIONAL_FORM for regional variants (Alolan/Galarian) whose
// enum tag carries a form prefix but maps to the base species in the National Dex.
static const enum NationalDexOrder sHoennToNationalOrder[HOENN_DEX_COUNT - 1] =
{
    ...
    HOENN_TO_NATIONAL_FORM(ALOLAN_VULPIX, VULPIX),
    HOENN_TO_NATIONAL_FORM(ALOLAN_NINETALES, NINETALES),
    // Powderpath Village + Powderpath Trail
    ...
    // R2 Icespire Pass + Icespire Town
    ...
    HOENN_TO_NATIONAL_FORM(ALOLAN_SANDSHREW, SANDSHREW),
    HOENN_TO_NATIONAL_FORM(ALOLAN_SANDSLASH, SANDSLASH),
    ...
    HOENN_TO_NATIONAL_FORM(GALARIAN_DARUMAKA, DARUMAKA),
    HOENN_TO_NATIONAL_FORM(GALARIAN_DARMANITAN, DARMANITAN),
    // Pinegrove City (Fishing/Surf)
    ...
    // R5 Ironfrost Cave + Ironfrost City Museum
    ...
    // R7 Galetop Plateau + Frostbreak Lodge
    ...
    // R8 Dreamurs Valley + Dreamurs Town
    ...
    // R9 Iceharbor Bay + Iceharbor City + Driftrock Isle
    ...
    // R12 Memoria Passage + Solace Town
    ...
    // R15 Pyrespire Lagoon + Pyrespire City Museum
    ...
    // Dragonforge / Facility Alpha gifts
...
u16 GetBattleBGM(void)
{
    ...
    else if (gBattleTypeFlags & BATTLE_TYPE_TRAINER)
    {
        ...
        case TRAINER_CLASS_RIVAL:
            ...
            return MUS_HG_VS_RIVAL; // Asher (default rival) — Silver theme from HGSS
    ...
    {
        // Snow wild encounter music — DPP wild theme matches the DPP route trainer theme tonally.
...
// Returns the Boralyss form bit for collision-pair species (Vulpix, Ninetales,
// Darumaka, Darmanitan — regular and regional-variant forms), or 0 if the
// species is not in the collision set.
static u8 GetBoralyssFormBitForSpecies(u16 species)
{
    switch (species)
    {
    case SPECIES_VULPIX:           return BORALYSS_FORM_BIT_VULPIX_REGULAR;
    case SPECIES_VULPIX_ALOLA:     return BORALYSS_FORM_BIT_VULPIX_ALOLA;
    case SPECIES_NINETALES:        return BORALYSS_FORM_BIT_NINETALES_REGULAR;
    case SPECIES_NINETALES_ALOLA:  return BORALYSS_FORM_BIT_NINETALES_ALOLA;
    case SPECIES_DARUMAKA:         return BORALYSS_FORM_BIT_DARUMAKA_REGULAR;
    case SPECIES_DARUMAKA_GALAR:   return BORALYSS_FORM_BIT_DARUMAKA_GALAR;
    case SPECIES_DARMANITAN:       return BORALYSS_FORM_BIT_DARMANITAN_REGULAR;
    case SPECIES_DARMANITAN_GALAR: return BORALYSS_FORM_BIT_DARMANITAN_GALAR;
    }
    return 0;
}
...
void HandleSetPokedexFlagFromMon(struct Pokemon *mon, u32 caseId)
{
    ...
    // Track Boralyss collision pairs (Alolan/Galarian variants + base forms)
    // independently of the National Dex flag so each slot tracks its own form.
    formBit = GetBoralyssFormBitForSpecies(species);
    if (formBit != 0)
    {
        if (caseId == FLAG_SET_SEEN)
            gSaveBlock2Ptr->pokedex.boralyssFormSeen |= formBit;
        else if (caseId == FLAG_SET_CAUGHT)
            gSaveBlock2Ptr->pokedex.boralyssFormCaught |= formBit;
    }
}
