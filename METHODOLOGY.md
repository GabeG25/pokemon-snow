...
Non-ASCII chars in .string literals break the build
The game's charmap.txt supports only ASCII + a handful of Pokemon-specific glyphs. Unicode punctuation (em-dash U+2014, en-dash U+2013, curly quotes U+2018-201D, ellipsis U+2026) inside a `.string "..."` literal causes preproc to fail with "unknown character U+XXXX" and halt make. Comments (lines starting with @) are stripped before the charmap check and are SAFE — only literal-content lines need ASCII discipline. Happened twice during Phase 2 interior build-out (Dragonforge Dept Store floor sign + Pinegrove PC Botanist NPC). Prevention: audit_scripts.py check_snow_scripts_charset() scans every Snow map's scripts.inc for these chars inside .string literals and flags at HIGH severity. Run audit after any new dialogue addition.
MAP_*_CHAMPIONS_ROOM enum collision with FRLG
FRLG's Indigo League defines MAP_POKEMON_LEAGUE_CHAMPIONS_ROOM in include/constants/map_groups.h (group 35). If a Snow map is also named PokemonLeague_ChampionsRoom, the enum redeclares and the build fails with "redeclaration of enumerator" at include/constants/map_groups.h. Fix: rename the Snow map to a non-colliding name. The Snow League Champion room was renamed from "ChampionsRoom" → "TyrimsRoom" during Phase 5 scaffolding, matching the leader-name pattern used for E4 rooms (BrynnsRoom, VespersRoom, ReveriesRoom, WyattsRoom). Also applies generally to any Snow map whose name would collide with FRLG's PokemonLeague_{Loreleis,Brunos,Agathas,Lances,Champions}Room namespace.
Name-override-without-constant-rename pattern
When shipped trainer data contains a narrative override that the tool's spec-parser doesn't produce (e.g., v17 §20 lists grunts as "Team Veil Grunt" but commit 2dc698ff12 renamed specific grunts to KAEL/BRAN/NYLA/ORIN/LIRA in the Name: field for dialogue continuity), decoupling is the right approach — NOT changing the trainer ID constant. Pattern: add a dict like GRUNT_NAME_OVERRIDES in port_trainers.py keyed by (route, index) → story name. Store the override in a separate dataclass field (e.g., story_name: str | None). At emit time, use override for Name: line but keep the original class-derived name in self.constant. This preserves byte-identical round-trip AND the TRAINER_SNOW_R3_5_GRUNT constant that opponents.h uses, while letting the Name: show KAEL. Applied once for 5 Act 1 Veil grunts. Same pattern extends to boss-fight name drift (E4 names, Champion) when a v17 §5 header uses a generic label like "E4 BRYNN" but the parser falls back on "E4" — fix is a header-parsing regex update, not a constant-rename cascade.
FLAG_UNUSED block strategy
When allocating new FLAG_SNOW_* slots, the 0x20-0x5F block is packed (vanilla scripts consume 0x50-0x57 interleaved). Don't scatter Snow flags into single-slot gaps at 0x68 / 0x71 / 0xE9 / 0x1AA — those UNUSED slots may still be referenced by vanilla R/S vestigial scripts (e.g., FLAG_UNUSED_RS_LEGENDARY_BATTLE_DONE at 0x71 is explicitly used by CaveOfOrigin_UnusedRubySapphireMap3_OnTransition, and the linker fails if the symbol is renamed). Instead: find a consecutive block of ≥5 FLAG_UNUSED slots and allocate there. Verified safe blocks include 0x1DE-0x1E3 (facility caps + TM18 allocation) and 0x264-0x267 (E4 defeat flags). Always verify with a full `make` build after allocating — the linker catches missing-symbol cases that grep won't.
gen_layouts.py --only flag for non-destructive regen
Running `python3 tools/snow_port/gen_layouts.py` with no args regenerates ALL 31 Snow maps, which clobbers any maps that have received custom paint post-initial bootstrap (Dawnflake Town, SnowRoute1, Powderpath, R2, Icespire, R3 all have polished paint). Use `--only NAME` (repeatable) to regenerate a specific map without touching neighbors: `python3 tools/snow_port/gen_layouts.py --only SnowRoute13`. Essential for Phase 8 visual-polish sessions where specific routes need re-run. Dimensions may change (R13 went 40×140 → 32×50 on regen); verify connections still match after regen if the map has neighbors.
Streamed PCM voice macro must match sample format
When wiring a streamed-PCM song, the voicegroup .inc and the sample .bin header MUST agree on encoding. `voice_directsound` (type byte 0x00) plays raw 8-bit signed PCM directly. `voice_directsound_comp` (type byte 0x20 = TONEDATA_TYPE_CMP) routes the channel through the engine's DPCM decoder (SoundMainRAM_Unk1 + gDeltaEncodingTable) which expects 33-byte blocks of delta-encoded data. Mismatching them — voicegroup says comp but sample is raw 8-bit, or vice versa — produces silence (engine decodes raw PCM as if it were DPCM block headers, gets garbage step values, drives the channel to inaudible territory). Real incident: 2026-05-08 Urayne stream silently failed because voicegroup_urayne_stream.inc still said `voice_directsound_comp` from the prior session's DPCM experiment, but the sample had been re-encoded as raw 8-bit. Fixed by changing the voicegroup macro to `voice_directsound`. Catch: if a streamed song plays silent on title screen, the first thing to check is voicegroup macro vs sample header type field. Sample header type=0x0000 means raw 8-bit; non-zero means DPCM. Voicegroup must match.
Sample-precise loop length detection beats crossfade for stitched audio
For loop encoding of YouTube "extended" or stitched-loop audio sources (where the uploader concatenated N copies of the original song), the cleanest result is sample-precise loop-length detection plus zero crossfade — NOT a long crossfade with approximate boundaries. Recipe: render the source to mono 16-bit PCM at the GBA mixer's exact rate (e.g., 21024 Hz). Pick a 1–5 second reference window from deep inside the body (well past intro). Find the FIRST later position where the window matches by maximum cross-correlation. Coarse search at 50–200 sample stride to find approximate offset, then fine-search within ±200 samples at sample-stride to pin it down. Verify with a second reference window from a different position — both should give the same loop_length to the sample. Then encode loop_end = loop_start + loop_length_samples with crossfade=0 (the source is already continuous at exactly that interval). Real incident: 2026-05-08 Urayne went through 5+ iterations with 100ms–1000ms crossfades trying to find a clean loop point; CEO consistently heard the seam regardless of position. Switching to sample-precise length matching (90.811168 s for Urayne; verified consistent from two reference windows) with zero crossfade resolved it on first try. Why crossfade fails: a 1-second crossfade blends the END of the body's last second with the START of the body's first second, producing audible "stutter" if the music is rhythmic (different beat phases overlap). Sample-precise alignment with no crossfade gives a hard cut where the audio is already continuous in the source.
Avoid sub-multiple loop-length false positives via large reference windows
Autocorrelation with a small window (1–5 sec) on rhythmic music can lock onto INTERNAL phrase repetitions (12 sec motif within a 96 sec body) and report a sub-multiple of the real loop length. The candidate scores cluster at integer multiples (e.g., 36/48/60/72/96/120 s all score similarly). The real loop is the LONGEST candidate that has full musical structure — and that's hard to know algorithmically. Mitigation: when autocorrelation produces multiple strong candidates, ask the CEO to listen to a 3-minute MP3 preview of the source and identify timestamp where the song restarts naturally. The CEO's ear is authoritative for "where does the song loop." Then snap to the closest sample-precise correlation peak near the CEO's reported timestamp. Real incident: 2026-05-08 first Urayne encoding used 87.6 s loop length detected with a 5 s reference window; CEO heard a jarring seam. Re-running with 20 s reference window produced multiple candidates (72, 48, 36 s — all sub-multiples of internal motif). CEO listened to 3-min preview and reported loop at 1:36 = 96.0 s. Sample-precise refinement around 96 s gave 90.811 s (intro 4.88 s + body 91 s, total 96 s + intro). Final encode used those exact values — clean.
mGBA hard-caps ROMs at 32 MB; expansion attempts white-screen
mGBA 0.10.5 (and prior versions) refuses to boot ROMs that place data past 0x0A000000 (the WS1 mirror region) on standard pokeemerald-expansion carts. Symptom: ROM loads, screen goes white, no boot. Tested 2026-05-03 with linker LENGTH=64M and pokeemerald.gba file size 64 MB containing 33 MB of actual data — white-screened on launch. Root cause: GBA hardware address bus is 25 bits (0x08000000–0x09FFFFFF = 32 MB max addressable in WS0). 0x0A and 0x0C are MIRRORS of the same physical 32 MB on real cart hardware; mGBA enforces this. Going past requires bank-switching cart hardware (Top Gear Rally has it; pokeemerald-expansion doesn't). Practical implication: when running into 32 MB cap, the answer is to free space (species disable, sample sharing, content stripping), NOT to expand the linker LENGTH. Adding bank-switching to pokeemerald-expansion is a 1–2 week engineering project and breaks emulator portability for older mGBA builds — only worth it if the project's audio/content scope genuinely cannot fit in 32 MB after all reasonable strips.
Shared-script object_event references pass through audit
audit_scripts.py's object_event check (line ~100) verifies script references resolve within the map's own scripts.inc. Shared scripts that live in data/scripts/*.inc (e.g., Snow_PokeMart_EventScript_Clerk in data/scripts/snow_mart.inc) legitimately live outside the per-map scripts.inc and the audit must tolerate them. Current prefix whitelist: Common_, EventScript_, Std_, Snow_. If a new shared-script namespace is introduced (e.g., Boss_, League_), extend the whitelist in check_snow_maps().
