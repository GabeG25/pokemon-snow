...
Audio & Streaming
Streamed PCM for Colress and Urayne, m4a engine for everything else
Decision: The Colress battle theme (Xenon's F20 fight) and the Urayne battle theme (Tyrell's F27 fight) play back as streamed raw 8-bit PCM samples — actual recorded game audio from B2W2 and Pokémon Uranium respectively — bypassing the GBA m4a synthesizer entirely. All other Snow music (route BGM, gym BGM, other battle themes, fanfares, SFX) continues to use the m4a engine with sample-based voicegroups in the normal way.
Rationale: Two narrative-flagship encounters justify the cost. Xenon parallels Colress; Tyrell's late-game encounter is built around the Urayne theme. CEO requirement was that these specific tracks sound 1:1 with their original-game versions, which is impossible through the GBA m4a synthesizer's per-instrument soundfont reconstruction (heard during the m4a-port phase as "GBA-flavored cover of Colress, not Colress"). PCM streaming bypasses the soundfont — the GBA plays the bytes directly through DirectSound — at the cost of (a) ROM space (~2 MB per song at 21024 Hz × ~100 s × raw 8-bit) and (b) a one-track limitation per streamed song (no Pokémon cries layered through music; cries play through the regular m4a path on other channels). The cost is acceptable for two flagship moments; not acceptable for general use because it'd dwarf the ROM with one song per ~2 MB.
Alternatives considered: (a) DPCM compression to halve ROM cost — rejected; the 4-bit delta encoding produces audible graininess on transients (CEO heard it and called it grainy). (b) Stay with m4a-engine ports of NDS-original tracks — produced the "still bare" / "wrong" audio Kirk repeatedly rejected. (c) Implement custom GBA bank-switching to access >32 MB ROMs — investigated; mGBA hard-caps at 32 MB for non-MBC carts, and adding Top Gear Rally style bank-switch hardware to pokeemerald-expansion is a multi-week engineering project with worse emulator portability (verified by white-screening a 64 MB ROM build).
Locked in: Session 2026-05-06 (Colress streaming) and 2026-05-08 (Urayne streaming). Both songs deployed and CEO-verified on title screen.

Mixer at SOUND_MODE_FREQ_26758, maxChans=12
Decision: The m4a sound mixer runs at 26758 Hz output rate with 12 simultaneous channels. Both values are set in src/m4a.c:m4aSoundInit().
Rationale: Higher mixer rate gives more audio bandwidth (~13 kHz Nyquist vs ~6.7 kHz at 13379 Hz), which brings out perceptible clarity on streaming PCM samples. maxChans=12 (up from the engineered default of 8 used during the streaming experiment phase) gives the engine spare channels so Pokémon cries can preempt dense music tracks like the HGSS Silver theme at Asher's rival fight without contention. The CPU budget at 26758 Hz × 12 channels fits comfortably in mGBA (verified empirically; no audio dropout or game stutter).
Alternatives considered: 13379 Hz (vanilla pokeemerald) — sounded muffled on streamed Colress. 31536 Hz — minimal additional bandwidth gain, more CPU. 21024 Hz — was tried, slightly less headroom for streamed sample upsample-to-mixer-rate. 26758 was the highest standard rate that still left clear CPU room. maxChans=8 — caused cries to fail to preempt Asher's theme. maxChans=15 — historical experiment caused audio glitches and game freezes at lower mixer rates; 12 is a safe ceiling.
Locked in: Session 2026-05-06 (mixer rate raised from 13379 to 26758 across the streaming sessions; maxChans bumped from 8 to 12 to fix cry contention).

P_GEN_6 through P_GEN_9 species disabled
Decision: P_GEN_6_POKEMON, P_GEN_7_POKEMON, P_GEN_8_POKEMON, and P_GEN_9_POKEMON are all set to FALSE in include/config/species_enabled.h. Snow's regional dex uses 196 P_FAMILY_* flags spanning Gen 1–5 base species plus their cross-gen evolutions (e.g., Sylveon as Eevee evolution, Slowking as Slowpoke evolution) and regional/mega/gigantamax forms.
Rationale: Snow's 196-family inventory (per agent audit 2026-05-06) contains zero Gen 6+ base species. Cross-gen evolutions of Gen 1–5 families are covered automatically by the base family flag (e.g., P_FAMILY_BULBASAUR covers Mega Venusaur even with P_GEN_6_POKEMON=FALSE, since Mega Venusaur is keyed to the family, not the gen-introduction flag). Disabling Gen 6–9 freed approximately 13 MB of ROM space — most of it from `data/sound_data.o` (Pokémon cry samples) and `src/pokemon.o` (species stats, learnsets, sprite tables for the ~750 base species Snow doesn't use). This headroom enabled streaming Colress and Urayne without the ROM blowing past the 32 MB hardware cap.
Alternatives considered: (a) Strip Hoenn vanilla maps instead — investigated; structurally entangled with engine code (battle_main.c, item_use.c, link.c all reference Battle Frontier scripts), would require multi-day surgery. (b) Strip individual unused P_FAMILY_* within active gens — possible but tedious; the gen-level sweep gets the bulk of savings for one config flip per gen. (c) Keep all gens enabled and use ROM bank-switching — see streaming PCM decision above; not viable for emulator-only target.
Locked in: Session 2026-05-06 (Gen 8, 9 disabled) and 2026-05-08 (Gen 6, 7 disabled). All commits to dawnflake-town pending; ROM verified clean and CEO-verified runtime.
